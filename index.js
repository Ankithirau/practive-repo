...
app.get('/contact_us', (req, res) => {
    ...
    res.render('contact', { title: 'Contact Us',loggedIn:loggedIn });
...
app.get('/dashboard', (req, res) => {
    if (req.session.loggedIn) {
        axios.get('https://reqres.in/api/unkown')
        .then(response => {
          const result = response.data;
          const loggedIn = req.session.loggedIn??null;
          res.render('dashboard', { result:result,loggedIn:loggedIn });
        })
        .catch(error => {
...
app.post('/create-user', async (req, res) => {
    ...
    const schema = Joi.object({
        ...
        job: Joi.string().max(10).min(2).required(),
    });
    const { error } = schema.validate(req.body);
    if (error) {
        const errors = error.details.map(detail => detail.message);
        return res.render('./users/create', { formData: req.body, errors: errors });
    }
    ...
    return res.render('./users/create', { status: status,msg:msg });
});
